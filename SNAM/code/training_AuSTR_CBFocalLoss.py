...
def train(model, train_loader, val_loader, optimizer):
  ...
  for epoch in range(EPOCHS):
    ...
    for batch_idx, (pair_token_ids, mask_ids, seg_ids, y) in enumerate(train_loader):
      ...
      focal_loss = FocalLoss( per_cls_weights, gamma=1, device=cuda0)
...
for seed in seeds:
    ...
    for fold in [1,2,3,4,5]:
      ...
      sys.stdout = open("results/%s_seed%s_fold%s_CBFocalLoss.txt"%(data,seed,fold), "w")
      print("fold",fold)
      tuning_model=[]
      tuning_lr=[]
      tuning_F1=[]
      ...
      for lr in lr_list:
       ...
       output_dir="models/%s_seed%s_fold%s_CBFocalLoss_%lr"%(data,seed,fold,lr)
       ...
       cls_num=[len(train_df[train_df['stance']=='disagree']),len(train_df[train_df['stance']=='agree']),len(train_df[train_df['stance']=='other'])]
       print(cls_num)
       per_cls_weights=reweight(cls_num)
      ...
      tuning_df.to_csv("results/tuning_results/%s_seed%s_fold%s_CBFocalLoss_tuning.txt"%(data,seed,fold),sep="\t")
